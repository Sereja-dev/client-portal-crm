...
describe("benchmark-version.ts — 1.2.0 temporal-grounding bump", () => {
  test("BENCHMARK_DEFINITION_VERSION reflects the temporal-grounding bump and every bump since (currently 1.4.0 — Scorer / Expectation Repair, see benchmark-version.ts's own History)", () => {
    ...
    // of re-deriving it.
    assert.equal(BENCHMARK_DEFINITION_VERSION, "1.4.0");
  });
